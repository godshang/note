...
构造器注入适合必需依赖，可以让对象在构造完成后即处于有效状态，也便于测试。它并不是到了 Spring 4.x 才统一取代其他注入方式；字段、setter 和构造器注入各有语义，选择应由依赖是否必需、可变以及是否需要解决循环依赖等因素决定。
...
Setter 注入适合可选或需要在对象生命周期中重新配置的依赖；必需依赖通常更适合构造器注入。不要仅按 Spring 大版本把某一种方式描述为唯一推荐方式。
