...
我们常见到各类秒杀系统设计的文章，但是秒杀只是扣减类业务中一个有代表性、具备一定技术复杂度的场景，它并不代表扣减类业务的全部场景。
...
上述业务场景有几个共性点：一次需要扣减的对象可以是一个或多个，数量可能在用户之间或同一用户的多个请求之间共享，每个请求都会减少某项数据的可用数量。基于上述分析，可以给扣减业务下一个定义：
...
那是不是前置校验就不需要了呢？在读请求成本可控、且前置校验能过滤大量无效写入时，可以保留前置校验。它只能作为快速失败的优化，最终扣减仍必须在数据库事务或原子操作中再次校验库存，不能依赖前置读取保证正确性。此外，扣减服务提供的数量查询接口和校验中的反查底层实现可能相同，如果反查走库则都要计入数据库容量。
