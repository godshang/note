...
但对于有些业务，比如协作文档编辑、电商秒杀扣库存、银行转账等，以上做法可能不够。常见思路之一是把数据库作为唯一权威数据源，缓存只用于可容忍陈旧读取的派生数据，关键写入和校验直接落在数据库事务中。这与“选择 CP、牺牲 A”不能简单画等号：CAP 讨论的是发生网络分区时分布式系统对一致性与可用性的取舍，单库方案的停机原因也不能直接归因于 CAP。

CAP 讨论的是发生网络分区时，一致性与可用性之间的取舍。没有分区时，系统可以同时提供一致性和可用性；工程上还会通过超时、复制协议和一致性级别，在延迟、可用性与一致性之间做更细粒度的权衡。
...
3. 使用基于共识协议的协调服务可以为锁或主节点选举提供可靠顺序，但“拿分布式锁后同时写 Redis 和数据库”仍不是原子提交：客户端可能在完成其中一次写入后崩溃。若确需跨资源原子性，应使用参与方都支持的事务协议；更多缓存场景则采用数据库提交后删缓存、CDC/binlog 驱动失效、重试和幂等，把不一致窗口控制在业务可接受范围内。
...
* 若要求缓存读始终返回最新数据，就必须引入能把数据库写入与缓存可见性纳入同一一致性协议的机制，成本通常很高；仅靠普通的数据库与 Redis 双写无法获得原子性。多数业务因此采用 Cache Aside、失效重试、CDC/binlog 和过期时间，接受短暂的不一致。真正需要强一致的读路径可以绕过缓存，或使用具备相应一致性保证的存储系统。发生网络分区时，系统仍需在一致性和可用性之间做选择。
