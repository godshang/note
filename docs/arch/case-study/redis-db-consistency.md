...
缓存与数据库属于两个独立系统，普通双写无法天然提供原子性，因此需要明确可接受的一致性级别，并通过写入顺序、过期时间、重试和监控来缩短或发现不一致窗口。
...
一种缓解方式是**延时双删**，但它依赖时序估算，不能严格保证一致性。
...
先淘汰缓存，再写数据库；延迟一段时间后再次淘汰缓存，可以清理部分并发读回填的旧值。示例中的 1 秒不是通用配置，而且在请求线程中直接休眠会降低吞吐量。

**那么，这个1秒怎么确定的，具体该休眠多久呢？**

延迟时间至少要覆盖读请求、主从复制和队列调度等可能的尾延迟，但这些延迟会波动，因此“耗时加几百毫秒”仍只是经验值。第二次删除还必须具备持久化重试、幂等和失败告警；严格一致性场景不应依赖固定休眠。
...
5. 请求B将旧值写入缓存
6. 数据库完成主从同步，从库变为新值。上述情形就是数据不一致的原因。延时双删只能降低发生概率；更稳妥的做法包括让回源读主库、等待复制位点追平，或在一致性窗口内避免从库旧值回填缓存。固定等待时间无法覆盖所有复制延迟。

**采用这种同步淘汰策略，吞吐量降低怎么办？**

可以将第二次删除交给持久化消息队列或调度系统异步执行。不要只启动进程内线程：进程退出、任务拒绝或重启都可能让删除任务丢失。异步执行还需要重试、死信和监控。
...
这样仍可能产生脏数据。这个竞态窗口通常较窄，但不能用“数据库读一定比写快”排除；查询抖动、锁等待、网络延迟都可能改变完成顺序。应为缓存设置合理过期时间，并让删除失败可重试。需要更强保证时，可让回填携带版本号并只写入未被更新覆盖的版本。
