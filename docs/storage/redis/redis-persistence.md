...
Redis 的工作数据主要位于内存中，进程退出后，未持久化的数据会丢失。RDB 和 AOF 可以降低数据丢失范围，但不能在所有故障下保证零丢失；实际保证取决于持久化策略、刷盘策略、存储设备和备份方案。
...
随着父进程持续修改数据，越来越多的共享页可能被复制，内存占用随之增长。额外占用与快照期间被写脏的内存页、页大小以及内核行为有关，不能简单保证总内存不超过原数据集的两倍；规划容量时应为 fork 和写时复制预留足够余量。
...
Redis 执行写命令后，会把相应协议内容追加到 AOF 缓冲区，并根据 `appendfsync` 策略将数据写入操作系统及刷到持久化存储。写入 AOF 缓冲区不等于已经落盘：`everysec` 策略通常可能丢失约 1 秒的数据，`always` 更安全但开销更大，`no` 则由操作系统决定刷盘时机。
...
是否在主节点启用持久化要根据数据安全目标与性能预算决定，不能一概关闭。只依赖从节点持久化仍可能在主从同时故障、复制延迟或错误故障转移时丢失数据；生产环境通常还需要独立备份，并定期验证恢复流程。
