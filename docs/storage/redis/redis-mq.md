...
使用 List 的 `LPUSH/RPOP` 或 `BLPOP/BRPOP` 实现时，队列本身没有消费组；如果消费者取出消息后在处理完成前崩溃，还会丢失该消息。可以通过 `BRPOPLPUSH`/`BLMOVE` 配合处理中列表自行实现确认与重试。Redis Streams 则原生支持消费组、待处理消息列表和显式确认，但仍需要设计重试、幂等、死信和持久化策略。
...
阻塞读在队列没有数据时会等待，一旦数据到来便被唤醒。用 `BLPOP/BRPOP` 替代 `LPOP/RPOP` 可以避免空轮询并显著降低空队列时的延迟，但它并没有解决消息确认、消费者崩溃后重投等可靠性问题。
