...
* 常数复杂度获取字符串长度。
* 拒绝C字符串缓冲区溢出。
* 减少修改字符串时触发内存重分配的次数。使用空间预分配、惰性空间释放两种优化策略。
* 二进制安全。
* 兼容部分C字符串函数。
...
* 哈希表的实现类似 `HashMap`，有一个 `table` 数组存储 `dictEntry`，`size` 属性记录哈希表大小（即 `table` 数组长度），`used` 属性记录已有键值对数量，`sizemask` 属性等于 `size - 1`，用于计算键值对所在的索引。
