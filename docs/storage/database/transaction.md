...
ACID 原子性描述了包含多个写操作的事务在故障时应如何处理。例如完成部分写入后，系统可能发生进程崩溃、磁盘空间耗尽或违反完整性约束等问题；如果事务无法完成最终提交，数据库必须丢弃或撤销那些局部完成的更改。网络中断只会让客户端无法确定提交结果，事务在数据库端可能已经提交，因此客户端还需要通过事务标识、幂等设计或状态查询来消除这种不确定性。
