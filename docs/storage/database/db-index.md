...
* 在普通索引列上使用函数或表达式，往往会使优化器无法直接使用该列索引进行定位；MySQL 8.0 的函数索引或与表达式匹配的生成列索引属于例外。
* 对索引列进行运算通常也有类似影响，实际是否使用索引应以 `EXPLAIN` 和真实数据分布为准。
* 联合索引不满足最左前缀时，通常无法使用缺失前导列之后的列做常规范围定位；但仍可能使用索引覆盖、索引条件下推，或在支持的版本中使用 skip scan，因此不能笼统写成“索引一定失效”。
