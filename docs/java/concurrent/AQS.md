...
同步队列遵循 FIFO。首节点是成功获取同步状态的节点；首节点对应线程释放同步状态时，会唤醒后继节点；后继节点在成功获取同步状态后，会将自己设置为首节点。由于同一时刻只有一个线程能够成功获取独占同步状态，因此设置首节点的方法不需要使用 CAS，只需要将首节点设置为原首节点的后继节点，并断开原首节点的 `next` 引用即可。
