...
<img src="./image/single-thread-rule.png" />

2. 管程锁定规则 (Monitor Lock Rule)

一个 unlock 操作先行发生于后面对同一个锁的 lock 操作。

<img src="./image/monitor-lock-rule.png" />

3. volatile 变量规则 (Volatile Variable Rule)

对一个 volatile 变量的写操作先行发生于后面对这个变量的读操作。

<img src="./image/volatile-variable-rule.png" />

4. 线程启动规则 (Thread Start Rule)

hread 对象的 start() 方法调用先行发生于此线程的每一个动作。

<img src="./image/thread-start-rule.png" />

5. 线程加入规则 (Thread Join Rule)

<img src="./image/thread-join-rule.png" />
