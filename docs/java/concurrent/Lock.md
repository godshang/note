...
本文对 Java 中常见锁概念进行归纳，重点说明不同术语的分类维度、适用场景和基本实现思路。

需要注意，一个具体的锁实现可以同时属于多个分类。例如，某个锁可以同时是悲观锁、可重入锁、公平锁和可中断锁。这些概念来自不同分类维度，并不互斥。

## 零、synchronized与Lock

Java 中常见的加锁方式包括使用 `synchronized` 关键字，以及使用 `Lock` 接口的实现类。

`synchronized` 是 Java 语言层面的内置同步机制，使用方式较简单，由虚拟机负责加锁和释放锁。`Lock` 是 `java.util.concurrent.locks` 包中的接口，要求显式获取和释放锁，并提供更灵活的能力。

如果只需要基本互斥语义，`synchronized` 通常已经足够。自 Java 5 起，JDK 在 `java.util.concurrent.locks` 包中提供了 `Lock` 接口及其实现。与 `synchronized` 相比，`Lock` 可以支持可中断获取、尝试获取、超时获取以及多个条件队列等能力。许多 `Lock` 实现基于 `AbstractQueuedSynchronizer` 构建。

<img src="./image/v2-ddb71ab0b68d65ae70244bfdeb0d6704_r.jpg" />

常见实现包括 `ReentrantLock`，以及 `ReentrantReadWriteLock` 中的 `ReadLock` 和 `WriteLock`。它们分别对应可重入互斥锁、读锁和写锁。

ReadWriteLock其实是一个工厂接口，而ReentrantReadWriteLock是ReadWriteLock的实现类，它包含两个静态内部类ReadLock和WriteLock。这两个静态内部类又分别实现了Lock接口。

下面从使用角度梳理锁的主要分类，以及 `synchronized` 与常见 `Lock` 实现之间的区别。

## 一、悲观锁与乐观锁

锁的一种宏观分类方式是悲观锁和乐观锁。悲观锁与乐观锁并不是特指某个锁（Java中没有哪个Lock实现类就叫PessimisticLock或OptimisticLock），而是在并发情况下的两种不同策略。

悲观锁（Pessimistic Lock）假定共享数据存在并发修改风险，因此访问共享数据前先获取锁，其他线程必须等待锁释放后才能继续访问受保护的数据。

乐观锁（Optimistic Lock）假定冲突较少，读取阶段通常不加互斥锁；更新时检查数据是否在读取后被其他线程修改。如果发生冲突，则重试或放弃操作。
...
CAS 是 Compare-and-Swap，也称 Compare-and-Set，含义是比较并交换或比较并设置。
...
自旋锁是指线程在无法立即获得锁时，不立即阻塞或挂起，而是在循环中反复尝试获取锁。

乐观锁实现中也可能包含循环重试，但不应简单等同于自旋锁。自旋锁强调的是等待锁时持续尝试获取锁；乐观锁强调的是无锁读、冲突检测和失败重试。

JDK 并未直接提供名为 `SpinLock` 的标准锁类，但虚拟机或并发算法内部可能使用短暂自旋来降低线程挂起与唤醒成本。

## 四、synchronized锁升级：偏向锁 → 轻量级锁 → 重量级锁

在早期 HotSpot 实现中，`synchronized` 可能经历无锁、偏向锁、轻量级锁、重量级锁等状态变化。需要注意，偏向锁在较新的 JDK 中已经被废弃并默认关闭，因此该部分应理解为特定 HotSpot 版本下的实现细节，而不是 Java 语言规范的固定要求。
...
忙等通常会受到限制。如果锁竞争严重，自旋失败的线程可能导致锁膨胀为重量级锁。后续线程尝试获取重量级锁时，会进入阻塞状态，等待后续唤醒。具体自旋策略和锁升级细节属于虚拟机实现细节，随 JDK 版本和运行参数可能有所不同。
...
>在升级为轻量级锁前，虚拟机通常需要在安全点撤销偏向状态，并根据线程栈和锁记录恢复到适当的锁状态。该过程属于 HotSpot 实现细节，不同 JDK 版本可能存在差异。

## 五、可重入锁（递归锁）

可重入锁的字面意思是“可以重新进入的锁”，即允许同一个线程多次获取同一把锁。比如一个递归函数里有加锁操作，递归过程中这个锁会阻塞自己吗？如果不会，那么这个锁就是可重入锁（因为这个原因可重入锁也叫做递归锁）。

Java 中常见的 `ReentrantLock` 和 `ReentrantReadWriteLock` 都是可重入锁，`synchronized` 也是可重入的。可重入性可以避免同一线程在递归调用或嵌套调用中再次获取同一把锁时发生自我死锁。
...
JDK 并发包中的 `java.util.concurrent.atomic` 原子类通常基于 CAS 实现，体现了乐观并发控制思想。

<img src="./image/v2-98cd919fe09521bac03aa66d6968aeb2_r.jpg" />

部分资料会将偏向锁、轻量级锁归类为乐观锁，原因通常是它们在底层实现中使用了 CAS。但从应用层同步语义看，线程在进入受保护临界区前仍需要获得锁；如果锁被占用，线程无法继续访问受保护数据。因此，将 `synchronized` 的这些锁状态理解为悲观锁实现更为合适。CAS 在这里主要用于优化锁获取过程，并不改变其互斥访问语义。

因此，理解锁的运行机制比单纯记忆术语更重要。分析具体锁实现时，应同时关注其互斥策略、阻塞方式、可重入性、公平性、中断响应能力以及对应的 JDK 版本。
