...
在I/O编程过程中，当需要同时处理多个客户端接入请求时，可以利用多线程或者I/O多路复用技术进行处理。I/O多路复用技术通过把多个I/O的阻塞复用到同一个select的阻塞上，从而使得系统在单线程的情况下可以同时处理多个客户端请求。与传统的多线程/多进程模型比，I/O多路复用的最大优势是系统开销小，系统不需要为每个连接创建额外进程或者线程，也不需要维护大量线程的运行，降低了系统的调度和内存开销，主要应用场景如下：
...
I/O 多路复用等待的是就绪事件，事件到达后仍由应用程序主动完成读写，因此属于同步 I/O；它与由内核完成整个读写并在完成后通知应用的 AIO 不同。等待 `select`、`poll` 或 `epoll_wait` 时可以阻塞，也可以通过超时或非阻塞方式轮询。
