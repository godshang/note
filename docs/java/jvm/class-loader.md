...
<img src="./image/e974afe6145e243c3673c817f7d90b90.png" />
...
《Java虚拟机规范》对这三点要求并未限定具体实现，给虚拟机实现与 Java 应用保留了较大灵活性。例如，“通过一个类的全限定名获取定义此类的二进制字节流”并未规定二进制字节流必须来自某个 Class 文件，也未限定获取位置和方式。因此，类加载阶段可以支持多种来源：

* 从ZIP压缩包中读取，这很常见，最终成为日后JAR、EAR、WAR格式的基础。
* 从网络中获取，早期典型场景包括 Web Applet。
* 运行时计算生成，典型场景包括动态代理技术。在 `java.lang.reflect.Proxy` 中，可以为特定接口生成形式为 `$Proxy` 的代理类二进制字节流。
...
JDK 8 及以前的 HotSpot 通常提供以下三类主要类加载器：

* 启动类加载器（Bootstrap ClassLoader）：通常由虚拟机实现，负责加载 Java 核心类库。
* 扩展类加载器（Extension ClassLoader）：JDK 8 及以前用于加载扩展目录中的类库；JDK 9 模块化后该机制已发生变化。
* 应用程序类加载器（Application ClassLoader）：也称系统类加载器，负责加载用户类路径（classpath）上的类库。若应用程序未自定义类加载器，通常由它加载应用类。
...
像 `java.lang.Object` 这类核心类型，无论请求来自哪个类加载器，最终通常都会委派给最顶端的启动类加载器加载，从而保证 Java 类型体系中基础类型的一致性。
...
<img src="./image/137084-20180526104342525-959933190.png" />
