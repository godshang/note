...
Java 各内存区域中，程序计数器、虚拟机栈、本地方法栈 3 个区域的生命周期与线程一致，栈帧需要分配的内存基本在类结构确定后即可知，因此这些区域的内存分配和回收具备较强确定性。Java 堆和方法区的内存分配与回收则更具动态性，垃圾收集器主要关注这部分内存。
...
* 虚引用：是最弱的一种引用关系。一个对象是否存在虚引用，不会影响其生存时间，也无法通过虚引用取得对象实例。为对象设置虚引用的主要目的，是在对象被回收时配合引用队列接收通知。
...
如果这个对象被判定为确有必要执行 `finalize()` 方法，那么该对象会被放入名为 F-Queue 的队列中，并在稍后由虚拟机自动建立的低调度优先级 Finalizer 线程执行其 `finalize()` 方法。虚拟机会触发该方法执行，但不保证等待其运行结束。`finalize()` 方法是对象逃脱回收的最后机会；稍后收集器会对 F-Queue 中的对象进行第二次小规模标记，如果对象在 `finalize()` 中重新与引用链上的任意对象建立关联，就可能避免本次回收。

注意，`finalize()` 方法不等同于 C/C++ 中的析构函数。它运行成本高、不确定性强，也无法保证各对象的调用顺序，目前已经被官方明确声明为不推荐使用的机制。
...
* 通过参数 `-XX:GCTimeRatio` 设置吞吐量目标，即垃圾回收时间占总运行时间的比例。
* 通过参数 -XX:MaxGCPauseMillis 设置垃圾处理过程最久停顿时间。
* 通过参数 `-XX:+UseAdaptiveSizePolicy` 开启自适应策略。设置堆大小、`MaxGCPauseMillis` 或 `GCTimeRatio` 后，收集器会尝试自动调整新生代大小、Eden 与 Survivor 比例、对象晋升年龄等参数，以接近设定目标。
...
CMS（Concurrent Mark Sweep，并发标记清除）以降低停顿为目标，大部分标记和清除工作可与用户线程并发，但初始标记、重新标记等阶段仍需要 Stop The World。实际停顿还受堆大小、对象存活率、CPU 竞争和并发失败影响，不能保证用户一定感受不到卡顿。
...
* G1 允许用户设置期望的最大停顿目标，收集器会据此选择回收集，在吞吐量与延迟之间进行权衡；该值是软目标而不是停顿上限保证。
...
使用 `-XX:MaxTenuringThreshold` 设置对象晋升老年代的最大年龄，达到该年龄阈值的对象会被转移到老年代。
...
3. 方法区或元空间不足
在 JDK 8 以前的 HotSpot 中，方法区主要由永久代（Permanent Generation）实现，可能因加载类过多、反射或动态生成类过多而触发 Full GC；如果回收后仍无法满足分配需求，可能抛出 `PermGen space` 相关错误。JDK 8 及以后永久代被元空间取代，类似问题通常表现为 `Metaspace` 相关的 `OutOfMemoryError`。
