...
JDK 8 相比 JDK 7 的重要变化之一是：HotSpot 使用元空间（Metaspace）取代永久代（Permanent Generation）来实现方法区。元空间与永久代都可视为 HotSpot 对 JVM 规范中方法区的具体实现方式，但元空间使用本地内存，而永久代使用虚拟机管理的内存区域。
...
通常所说的“堆”和“栈”中的“栈”，一般指 Java 虚拟机栈，很多场景下进一步特指栈帧中的局部变量表。局部变量表存放编译期可知的 Java 虚拟机基本数据类型、对象引用和 `returnAddress` 类型。

Java 虚拟机栈的特点：

* 局部变量表随着栈帧的创建而创建，它的大小在编译时确定，创建时只需分配事先规定的大小即可。在方法运行过程中，局部变量表的大小不会发生改变。
* Java 虚拟机栈可能出现两种异常：`StackOverflowError` 和 `OutOfMemoryError`。
...
本地方法栈与虚拟机栈的作用非常类似，区别只是虚拟机栈为执行 Java 方法服务，而本地方法栈为虚拟机使用到的本地 Native 方法服务。
...
* 在虚拟机启动时创建。
* 是垃圾收集器管理的内存区域。
* 在基于分代收集理论的收集器中，可进一步划分为新生代（Eden、From Survivor、To Survivor）和老年代。

堆的大小既可以固定也可以扩展，但对于主流的虚拟机，堆的大小是可扩展的，因此当线程请求分配内存，但堆已满，且内存已无法再扩展时，就抛出 OutOfMemoryError 异常。

Java 对象实例通常分配在堆上。但随着即时编译和逃逸分析技术的发展，部分对象可能通过标量替换等优化被消除实际分配，或在实现层面表现为不再进入堆。
...
* 线程共享。方法区是各线程共享的运行时内存区域。
* 实现方式。永久代是 JDK 8 以前 HotSpot 对方法区的一种实现方式；JDK 8 及以后 HotSpot 使用元空间实现方法区。方法区不能简单等同于永久代。
* 内存回收效率低。 方法区中的信息一般需要长期存在，回收一遍之后可能只有少量信息无效。主要回收目标是：对常量池的回收；对类型的卸载。
* Java 虚拟机规范对方法区的要求比较宽松。 和堆一样，允许固定大小，也允许动态扩展，还允许不实现垃圾回收。

方法区与永久代常常被混为一谈，本质上两者并不等价。永久代只是早期 HotSpot 对方法区的一种实现；其他虚拟机实现未必存在永久代。JDK 7 的 HotSpot 已将字符串常量池等内容移出永久代，JDK 8 则废弃永久代，改用本地内存中的元空间保存类元数据。类的静态字段随 `java.lang.Class` 对象位于 Java 堆中，不能笼统表述为“静态变量存放在元空间”。
...
* 直接内存的分配和释放成本通常高于普通堆内存。
* 在合适场景下，直接内存可以减少 Java 堆与本地内存之间的数据复制，从而提升 I/O 性能。
...
本节说明 HotSpot 虚拟机中对象分配、布局和访问定位的基本过程。
...
1. 如果堆中内存是规整的，所有已使用内存位于一边，空闲内存位于另一边，中间用指针标记分界位置，那么分配内存只需把指针向空闲方向移动一段与对象大小相等的距离，这种方式称为“指针碰撞”。
...
另一个问题是并发情况下如何保证对象分配的线程安全。常见解决方案有两种：一种是对分配内存空间的动作进行同步处理，例如使用 CAS 配合失败重试保证原子性；另一种是使用本地线程分配缓冲区（TLAB），提前为不同线程划分私有分配区域，只有本地缓冲区用尽并申请新缓冲区时才需要同步。
...
对象头包括两类信息：第一类用于存储对象自身的运行时数据，如哈希码、GC 分代年龄、锁状态标志、线程持有的锁、偏向线程 ID、偏向时间戳等；第二类是类型指针，即对象指向其类型元数据的指针。此外，如果对象是 Java 数组，对象头中还必须包含记录数组长度的数据。
